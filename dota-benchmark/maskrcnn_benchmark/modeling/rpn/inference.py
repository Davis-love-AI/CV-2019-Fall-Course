...
class RPNPostProcessor(torch.nn.Module):
    ...
    def add_gt_proposals(self, proposals, targets):
        ...
        if cfg.ROTATE:
            if cfg.RECT_POLY_BALANCE == "Rot":
                gt_boxes = [target.copy_with_fields(["xywht", "xyxy"]) for target in targets]
            elif cfg.RECT_POLY_BALANCE == "Rect":
                gt_boxes = [target.copy_with_fields(["xyxy"]) for target in targets]
                for gt_box in gt_boxes:
                    xyxy = gt_box.get_field("xyxy")
                    xmin, ymin, xmax, ymax = xyxy.split(1, dim=-1)
                    gt_box.bbox = torch.cat((xmin, ymin, xmax, ymin,
                                             xmax, ymax, xmin, ymax), dim=1)
                    xywht = torch.cat(
                        (
                            (xmin + xmax) / 2.,
                            (ymin + ymax) / 2.,
                ...
                gt_boxes_rects = [target.copy_with_fields(["xyxy"]) for target in targets]
                gt_boxes_rots = [target.copy_with_fields(["xywht", "xyxy"]) for target in targets]
                ...
                gt_boxes = [
                    ...
                    for gt_boxes_rect, gt_boxes_rot in zip(gt_boxes_rects, gt_boxes_rots)
                ]
    ...
    def forward_for_single_feature_map(self, anchors, objectness, box_regression):
        ...
        if cfg.ROTATE and "RETINANET" in cfg.MODEL.BACKBONE.CONV_BODY:
            ...
            proposals = self.box_coder.decode(
                box_regression.view(-1, 5), concat_anchors.view(-1, 4)
            )
            ...
            for proposal, score, im_shape in zip(proposals, objectness, image_shapes):
                if cfg.MODEL.RETINANET_DCN_ON:
                    ...
                    proposal = torch.cat((bbox_x1, bbox_y1, bbox_x2, bbox_y2,
                                          bbox_x3, bbox_y3, bbox_x4, bbox_y4), dim=0)
                else:
            ...
            for proposal, score, im_shape in zip(proposals, objectness, image_shapes):
                if cfg.ROTATE:
                    ...
                    proposal = torch.cat((xmin, ymin, xmax, ymin, xmax, ymax, xmin, ymax), dim=1)
    ...
    def forward(self, anchors, objectness, box_regression, targets=None):
        ...
        if self.training and targets is not None and cfg.RECT_POLY_BALANCE != "No":
