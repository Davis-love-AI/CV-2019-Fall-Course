        ...
        for per_box_cls, per_box_regression, per_pre_nms_top_n, \
            ...
            if cfg.ROTATE and "RETINANET" in cfg.MODEL.BACKBONE.CONV_BODY:
                detections = self.box_coder.decode(
                    per_box_regression[per_box_loc, :].view(-1, 5),
                    per_anchors.bbox[per_box_loc, :].view(-1, 4)
                )
                detections = trans.convert8(detections)
                boxlist = BoxList(detections, per_anchors.size, mode="xy8")
