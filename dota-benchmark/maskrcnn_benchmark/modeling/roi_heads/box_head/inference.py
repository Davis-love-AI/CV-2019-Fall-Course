...
class PostProcessor(nn.Module):
    ...
    def __init__(
        ...
        if box_coder is None:
            if cfg.ROTATE:
                box_coder = BoxCoder2(weights=(10., 10., 5., 5.))
            else:
                box_coder = BoxCoder2(weights=(10., 10., 5., 5., 10.))
    ...
    def forward(self, x, boxes):
        ...
        if cfg.ROTATE:
            concat_boxes = torch.cat([a.get_field("xywht") for a in boxes], dim=0)
        else:
    ...
    def prepare_boxlist(self, boxes, scores, image_shape, boxes_h=None):
        """
        ...
        if cfg.ROTATE:
            ...
            if cfg.MULTI_REG:
                x, y, w, h, t = boxes.split(1, dim=-1)
                boxes = torch.where(t >= -1.5708,
                                    torch.cat((x, y, w, h, t), dim=1),
                                    torch.cat((x, y, h, w, t + 1.5708), dim=1)
                                    )
            boxes = trans.convert8(boxes)
            scores = scores.reshape(-1)
            boxlist = BoxList(boxes, image_shape, mode="xy8")
    ...
    def filter_results(self, boxlist, num_classes):
        ...
        if cfg.ROTATE:
            ...
            for j in range(1, num_classes):
                ...
                boxlist_for_class = boxlist_rnms(
