...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if cfg.SNL_ATTENTION:
            if cfg.ADD_C2:
                Wk = self.Wk_layer(out)
                Wk = Wk.view(Wk.size(2) * Wk.size(3), 1)
                Wk = F.softmax(Wk, dim=0)
                Wv = out.view(-1, out.size(2) * out.size(3))
                Wv = Wv.mm(Wk)
                out = out + self.Wv_layer(Wv.view(out.size(0), -1, 1, 1))
            elif self.layers_ != 256:
                Wk = self.Wk_layer(out)
                Wk = Wk.view(Wk.size(2) * Wk.size(3), 1)
                Wk = F.softmax(Wk, dim=0)
                Wv = out.view(-1, out.size(2) * out.size(3))
        ...
        if cfg.GC_ATTENTION:
            if cfg.ADD_C2:
                ...
                out_input = out
                # [M, C, H*W]
                out_input = out_input.view(batch, channel, height * width)
                # [N, 1, C, H*W]
                out_input = out_input.unsqueeze(1)
                # [N, 1, H*W]
                Wk = self.Wk_layer(out).view(batch, 1, height * width)
                # [N, 1, H*W]
                Wk = self.softmax(Wk)
                # [N, 1, H*W, 1]
                Wk = Wk.unsqueeze(-1)
                # [N, 1, C, 1]
                context = torch.matmul(out_input, Wk)
                # [N, C, 1, 1]
