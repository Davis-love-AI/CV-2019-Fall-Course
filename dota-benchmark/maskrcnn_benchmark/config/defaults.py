...
_C.MODEL.RETINANET_DCN_ON = False
...
_C.MODEL.RPN.ATTENTION = "AttentionHead"
# Rpn cls and loc weight
_C.MODEL.RPN.CLS_WEIGHT = 1.
_C.MODEL.RPN.LOC_WEIGHT = 1.
# Rpn attention inner layers X*5--inner layer number--1
_C.MODEL.RPN.INNER_LAYER = 64
...
# Rpn cls and loc weight
_C.MODEL.ROI_HEADS.CLS_WEIGHT = 1.
_C.MODEL.ROI_HEADS.LOC_WEIGHT = 1.
...
# OR-ResNet
# ---------------------------------------------------------------------------- #
_C.MODEL.OR_RESNETS = CN()
# Number of orientation default: 8
_C.MODEL.OR_RESNETS.ORIENTATION = 8
# Number of groups to use; 1 ==> ResNet; > 1 ==> ResNeXt
_C.MODEL.OR_RESNETS.NUM_GROUPS = 1

# Baseline width of each group
_C.MODEL.OR_RESNETS.WIDTH_PER_GROUP = 64

# Place the stride 2 conv on the 1x1 filter
# Use True only for the original MSRA ResNet; use False for C2 and Torch models
_C.MODEL.OR_RESNETS.STRIDE_IN_1X1 = False  # True

# Residual transformation function
_C.MODEL.OR_RESNETS.TRANS_FUNC = "BottleneckWithFixedBatchNorm"
# ResNet's stem function (conv1 and pool1)
_C.MODEL.OR_RESNETS.STEM_FUNC = "StemWithFixedBatchNorm"

# Apply dilation in stage "res5"
_C.MODEL.OR_RESNETS.RES5_DILATION = 1

_C.MODEL.OR_RESNETS.BACKBONE_OUT_CHANNELS = 256 * 4
_C.MODEL.OR_RESNETS.RES2_OUT_CHANNELS = 256
_C.MODEL.OR_RESNETS.STEM_OUT_CHANNELS = 64

# ---------------------------------------------------------------------------- #
# DCN Options (Follow the Detectron version)
# ---------------------------------------------------------------------------- #
_C.MODEL.DCN = CN()

_C.MODEL.DCN.OFFSET = 5  # 18, 27

_C.MODEL.DCN.BRANCH_FACTOR = 8
...
_C.MODEL.RETINANET.CLS_WEIGHT = 1.
_C.MODEL.RETINANET.LOC_WEIGHT = 1.
...
_C.ONLY_MODEL_WEIGHT = False

_C.ROTATE = False

# reg_fuction
_C.FACTOR_IOU_LOSS = False
_C.MULTI_REG = False
_C.R2CNN = False

# attention module
_C.RPN_ATTENTION = False
_C.ADD_C2 = False
_C.SNL_ATTENTION = False
_C.GC_ATTENTION = False

_C.RECT_POLY_BALANCE = "No"  # "Rect" "Rect+Rot"
_C.FACTOR_PROPOSAL_GT = 0.5
_C.BALANCE_GT = False
